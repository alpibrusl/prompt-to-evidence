...
checks an analysis against this book's closing checklist before anyone reports a number to you as though it settled a question.
...
"can I take this number to the board?" and having something ask it for you every time.
...
The skill lives in this book's repository, at `skills/verify-evidence/`. Download
...
```
~/.claude/skills/verify-evidence/
```

That makes it available in every project. To scope it to one project
instead, copy it to `.claude/skills/verify-evidence/` inside that project.
...
```
~/.config/opencode/skills/verify-evidence/
```
