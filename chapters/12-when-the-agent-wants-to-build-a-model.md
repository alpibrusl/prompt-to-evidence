...
A **batch model** runs once, or on a schedule you control — once a month, say — in an environment you control, and produces a set of outputs, then stops until next time. "Which of our current customers look likely to churn" computed on the first of the month, handed to the marketing team as a list someone reviews before acting on it, is a batch model used the safe way. If the model is wrong this month, the damage is one wrong list, caught the next time a human reviews it, fixable by rerunning it.
...
The schedule and the review are two different things, and it's the review that actually protects you — not the schedule. A batch job that runs monthly but automatically suspends every account it flags, with nobody looking at the list first, has the risk profile of an online model wearing a batch model's clothes: wrong continuously and automatically until someone notices in the aggregate, same as if it ran live. The question worth asking isn't only "does this run on a schedule or continuously" — it's "does a person see the output before it takes effect, or does the output act by itself." Where those two answers point the same way — reviewed and batch, or automatic and live — the categories above hold. Where they don't, treat it by whichever answer is actually true: does something act on this without a person looking first.

This is not a technical footnote — it changes the entire risk category. A model whose output a person reviews first produces one wrong list that gets caught. A model whose output acts by itself is wrong continuously and automatically, at full speed, until someone notices something is off in the aggregate and intervenes. The blast radius, and how fast it accumulates, are not the same problem wearing different clothes; they are genuinely different problems.

Everything else in this chapter — testing honestly, comparing against a simple baseline, treating a suspiciously good result with suspicion — applies fully to both kinds of model, without exception. But an online model takes on a second, separate set of obligations once it goes live inside a running system: it needs watching for its inputs quietly changing shape over time, a way to roll it back the moment it starts behaving badly, the same operational discipline any live system needs. That discipline is the whole subject of *Prompt to Production*, not this one's — if a model is headed for that kind of deployment, treat it as a live system first and a model second, and read accordingly.
...
Chapter 7 promised this moment would arrive again, and here it is: a model reporting startling accuracy — 99%, "gets it right almost every time" — is, more often than it is a genuine triumph, a sign of one of two problems. The first is a leak: a feature that quietly encodes the answer, the same records appearing on both sides of the split, a leak somewhere between training and evaluation. The second, and the one worth checking first for something like the churn prediction that opened this chapter, is a lopsided outcome: if only 4% of customers actually churn in a given month, a model that predicts "nobody churns" — ever, for anyone, ignoring the input entirely — already scores 96% accuracy while catching not a single at-risk customer. A high number on an imbalanced outcome like churn tells you almost nothing on its own, leak or no leak. Treat a suspiciously high accuracy the way Chapter 7 said to treat any extreme result: check what a genuinely trivial guess — always predicting the more common outcome, the same "simplest possible approach" comparison a few sections below — would already score on the same number, and check for a leak, before you believe the accuracy is telling you what it sounds like it's telling you.
...
One thing the record does not fix on its own: comparing twenty attempts against the *same* held-out data to pick a winner, then reporting that same held-out score as the model's accuracy, has quietly let the model's selection process see that held-out data twenty times over — which is a version of the exact peeking problem the held-out split existed to prevent in the first place. The honest fix is to keep the comparison and the final number separate: use the held-out data to pick a winner among the attempts, then check the winner once more against one further, genuinely untouched slice nobody looked at during the comparison, and report that number. If there's no budget for a second slice, at minimum treat the winner's held-out score as somewhat optimistic in proportion to how many attempts competed for it — which is exactly the context the record makes it possible to have honestly, rather than the number arriving with no way to tell.
