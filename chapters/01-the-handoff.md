...
For most of the history of statistical analysis, producing a real regression or a real causal comparison required enough technical grounding that many of the surrounding cautions got picked up along the way — even once spreadsheets and statistical software put the calculation itself within reach, getting a *trustworthy* result still meant knowing what the method assumed, what could go wrong with it, and how to sanity-check what came out. Doing it at all tended to drag you up part of that hill first. The rigor came along for the ride, unevenly but usually, the same way a person who learned to drive a manual transmission absorbed something about how an engine works whether they meant to or not.
