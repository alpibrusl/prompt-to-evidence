...
Width is the part worth reading, more than the point estimate itself. A narrow interval means the data pins the answer down precisely — you have enough of it, and the estimate isn't moving around much. A wide interval means the opposite: the honest answer is "somewhere in a fairly large range," even though the single headline number reports one specific point inside it with exactly the same confident formatting either way.

One precise thing is worth saying about what a range like "95% confidence" actually claims, because the common shorthand for it — "there's a 95% chance the true value is in this range" — is not quite right, and this book would rather be exactly right than easy. What it actually claims is a statement about the method: if you repeated this same sampling process many times, about 95% of the intervals it produced would contain the true value. The practical difference rarely matters for a decision in front of you today; treat the interval as your genuine plausible range and act accordingly, and leave the precise philosophical distinction to a statistics course, exactly as Chapter 1 warned this one wouldn't be.

Sample size is what narrows it, and this is Chapter 2's noise again, in the form that finally makes the connection obvious: more data reduces noise, and a confidence interval is nothing but noise, measured and reported honestly as a range instead of hidden inside a single falsely-precise number. A test that ran on four hundred people will almost always report a wider interval than the same test run on forty thousand, even if the point estimate happens to land in the same place for both.
...
The practical rule: before acting on a number, ask whether the interval is narrow enough, relative to the decision on the table, to actually distinguish the options in front of you. If the interval spans the line between "worth doing" and "not worth doing" — including, most importantly, whether it spans zero — you do not yet have enough certainty to choose, regardless of which side of that line the point estimate happens to sit on. That is not a failure of the analysis. It is the analysis correctly reporting that more data is needed before this particular question has a real answer.
