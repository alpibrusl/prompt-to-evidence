...
That is worth sitting with for a moment, because it is not obviously true of an agent. A person running a regression by hand, or even in a spreadsheet they built line by line, absorbed a rough sense of where it could go wrong along the way — which is exactly the sense this book has spent its length trying to hand you without the years it usually takes to earn. An agent has something different: it can run the actual computation flawlessly, every time, and still have no more insight than you do into whether the comparison behind it was fair. Fluency and understanding came bundled in a person. They don't have to come bundled in a tool, and increasingly they don't.
...
What does not change, here or in the argument I make elsewhere, is where responsibility sits. An agent can compute a comparison flawlessly and still not be the one accountable for the decision built on it. That was never going to be delegated, and no improvement in the tooling moves it.
