...
A script written to answer one specific question has the answer's shape baked into its own logic. Ask for "average order value in March, for customers in Spain" and it is entirely possible to get back code that works, produces the right number for March and Spain, and would need to be rewritten — not re-run, rewritten — to answer the same question about April, or about France. That code is not a tool. It is the answer, dressed up as code, and it inherits every weakness a plain one-off answer has: nobody checked what it assumes, because there was never a second case to check it against.

A **parameter** is the part that separates cleanly: the values that change from one run to the next — the month, the country, the customer segment — kept apart from the logic that computes the answer once those values are supplied. "Average order value, for a given month and a given country" is a tool with two parameters. "Average order value in March, for Spain" is that same tool with the parameters already baked in and welded shut. The computation inside might be identical in both cases. Only one of them can be handed a different month without being rebuilt from scratch.
